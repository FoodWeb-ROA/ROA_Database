...
-- 7) RPCs used by duplicate resolver -------------------------------------------
-- Exact slug match for RAW ingredients (components)
DROP FUNCTION IF EXISTS public.find_ingredients_by_slug(text[], uuid);
CREATE OR REPLACE FUNCTION public.find_ingredients_by_slug(_slugs text[], _kitchen uuid)
RETURNS TABLE(slug text, ingredient_id uuid)
LANGUAGE sql STABLE SECURITY DEFINER
AS $$
  SELECT public.slug_simple(c.name) AS slug,
         c.component_id             AS ingredient_id
    FROM public.components c
   WHERE c.kitchen_id = _kitchen
     AND c.component_type = 'Raw_Ingredient'
     AND public.slug_simple(c.name) = ANY(_slugs);
$$;

-- Fuzzy slug match for RAW ingredients
DROP FUNCTION IF EXISTS public.find_ingredients_fuzzy(text[], uuid, real);
CREATE OR REPLACE FUNCTION public.find_ingredients_fuzzy(_slugs text[], _kitchen uuid, _threshold real DEFAULT 0.88)
RETURNS TABLE(input_slug text, ingredient_id uuid)
LANGUAGE sql STABLE
AS $$
WITH cand AS (
  SELECT s AS input_slug,
         c.component_id,
         similarity(public.slug_simple(c.name), s) AS sim,
         row_number() OVER (PARTITION BY s ORDER BY similarity(public.slug_simple(c.name), s) DESC) AS rn
    FROM unnest(_slugs) s
    JOIN public.components c
      ON c.kitchen_id = _kitchen
     AND c.component_type = 'Raw_Ingredient'
     AND similarity(public.slug_simple(c.name), s) >= _threshold
)
SELECT input_slug, component_id
  FROM cand
 WHERE rn = 1;
$$;

-- Fingerprint exact match (extended to all recipes having fingerprints)
DROP FUNCTION IF EXISTS public.find_preparations_by_fingerprints(uuid[], uuid);
CREATE OR REPLACE FUNCTION public.find_preparations_by_fingerprints(_fps uuid[], _kitchen uuid)
RETURNS TABLE(fingerprint uuid, preparation_id uuid)
LANGUAGE sql STABLE SECURITY DEFINER
AS $$
...
-- Fuzzy plain fingerprint match (extended; returns preparation_id, may be NULL for dishes)
DROP FUNCTION IF EXISTS public.find_preparations_by_plain(text[], uuid, real);
CREATE OR REPLACE FUNCTION public.find_preparations_by_plain(_names text[], _kitchen uuid, _threshold real DEFAULT 0.75)
RETURNS TABLE(fingerprint_plain text, preparation_id uuid, sim numeric)
LANGUAGE sql STABLE SECURITY DEFINER
AS $$
  WITH q AS (
  ...
  SELECT r.fingerprint_plain,
         c.component_id AS preparation_id,
         similarity(r.fingerprint_plain, q.plain) AS sim
    FROM q
    JOIN public.recipes r ON r.kitchen_id = _kitchen
    JOIN public.components c ON c.recipe_id = r.recipe_id AND c.recipe_id IS NOT NULL
   WHERE r.fingerprint_plain IS NOT NULL
     AND similarity(r.fingerprint_plain, q.plain) >= _threshold
   ORDER BY sim DESC;
$$;
