 ...
 BEGIN
   ...
   IF _recipe_id IS NOT NULL THEN
     ...
     ELSIF v_recipe_type = 'Preparation' THEN
       ...
       IF FOUND THEN
         ...
         WHERE public.components.component_id = v_existing_component_id;
     ...
     IF v_existing_type = 'Preparation' AND v_recipe_type = 'Dish' THEN
       ...
       WHERE public.components.recipe_id = v_recipe_id;
       v_component_id := NULL;
     END IF;
   ELSE
   ...
   IF _components IS NOT NULL AND jsonb_array_length(_components) > 0 THEN
     ...
     WHERE recipe_components.recipe_id = v_recipe_id;
