-- Clean up save RPCs now that trigger-based cleanup is removed
-- Remove the deferred cleanup calls since there are no more triggers to defer

-- Update upsert_preparation_components - remove deferred cleanup logic
...
'Atomically replaces a preparation''s recipe_components with validated items in one transaction. No trigger cleanup needed.';

-- Update replace_recipe_components - remove deferred cleanup logic
...
'Atomically replaces a recipe''s components in one transaction with access checks. No trigger cleanup needed.';
